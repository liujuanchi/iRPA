...
#存放列名
name_list = ['序号', '产品名称', '发行银行', '委托货币', '发行日', '停售日', '管理期(天)', '预期收益率', '到期收益率', '与同期储蓄比', '综合评级']
...
while t.read(element_identifier= '//*[@class = "pf-disabled"][last()]') != '下一页>>' or (page_curr == 1):
    ...
    for i in range(1,count_values):
        ...
        if str(t.read(element_identifier='//tbody[@id = "content"]//tr['+str(i)+']//td[@class = "px"]')) == str_to_append:
            ...
            #综合评级
            value_dict[name_list[10]].append(t.read(element_identifier='//tbody[@id = "content"]//tr[' + str(i) + ']/td[12]'))
...
today_data.to_csv(str_to_append+".csv",index=False)
